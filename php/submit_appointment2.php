...
if ($stmt->execute()) {
    ...
    $stmt_makeover_admin = $conn->prepare("INSERT INTO makeover_admin.tblcustomers (Name, MobileNumber) 
                                         VALUES (?, ?)");
    $stmt_makeover_admin->bind_param("ss", $fullname, $contact);
    $stmt_makeover_admin->execute();
    $stmt_makeover_admin->close();
} else {
